...
void chosen_class(T* q, int n_q) //шаблонная функция для дальнейшей работы с классом
{
	...
	int cnt = 0;				//переменная для выбора очереди
	...
	while (flag == 1)
	{
		...
		cout << "2 - Удаление элемента очереди" << endl;
		cout << "3 - Вывод очереди на экран" << endl;
		cout << "4 - Количество элементов больших, чем среднее гармоническое очереди" << endl;
		cout << "5 - Копирование очереди" << endl;
		...
		case 2: //удаление элемента очереди
			if (q[index].isEmpty())
			{
		...
		case 4: //Количество элементов больших, чем среднее гармоническое очереди
			if (q[index].isEmpty())
			{
				cout << "Очередь пуста.\n" << endl;
				system("pause");
			...
			else
			{
				...
				cout << "Количество элементов больших, чем среднее гармоническое очереди: " << res << endl;
				cout << "\n";
			}
			break;
		...
		case 5: //создание копии очереди
			...
			{
				cout << "В какую очередь скопировать \n" << endl;
				cin >> cnt;
				if ((cnt < 0) || (cnt > n_q))
				{
				...
				if (1 == n_q || cnt == index)
				{
					cout << "Невозможно создать копию очереди, некуда вставить\n" << endl;
					system("pause");
					break;
				}
					system("cls");
					q[cnt].copy(q[index]);
					cout << "Очередь успешно скопирована в очередь: №" << cnt << "\n" << endl;
			}
			break;
